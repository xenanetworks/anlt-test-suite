###############################################################
#                                                             #
#       COEFFICIENT BOUNDARY TEST - MAX LIMIT TEST            #
#                                                             #
# Objective                                                   #
# To make the test port to respond COEFF_AT_LIMIT when        #
# incrementing a coefficient on the remote transmitter.       #
#                                                             #
###############################################################
import asyncio
import logging
from xena_anlt_lib import coeff_boundary_max_limit_test
...
#---------------------------
# xena_lt_coeff_max_limit
#---------------------------
async def xena_lt_coeff_max_limit(chassis: str, test_port_str: str, preset: int, coeff: str, include_an: bool, use_pam4pre: bool):
    ...
    await coeff_boundary_max_limit_test(
        chassis_ip=chassis,
        module_id=_mid_test,
        port_id=_pid_test,
        username="trainer",
...
if __name__ == "__main__":
    asyncio.run(xena_lt_coeff_max_limit(
        chassis=CHASSIS_IP,
